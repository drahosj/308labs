...
{
#warning queue_push not finished
	...
	else
	{
		...
		{
			case QUEUE_HEAD:
				/// @todo push this node to the head and move the head
				
				break;
			case QUEUE_TAIL:
				/// @todo push this node to the tail and move the tail
...
{
#warning queue_pop not finished
	// assert the list is valid
	assert(list > 0);
	queue_node_t* node;
	void* data;
	
	/// @todo wait for the list to contain something
